...
const checks = {
  passingCandlesBecomeActive:
    service.includes("marketStatus: enough ? \"active\"") &&
    service.includes("providerStatus: enough ? \"available\"") &&
    service.includes("const lastError = enough ? null"),
  passingVerificationClearsOldProviderError:
    service.includes("last_error=$8") &&
    service.includes("enough ? null : lastFailure?.code") &&
  ...
  oldRateLimitDoesNotDemoteActiveMarkets:
    service.includes("providerError && isReadyStatus(current.status || current.marketStatus)") &&
