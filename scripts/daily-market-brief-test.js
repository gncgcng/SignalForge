...
assert.match(html, /daily-brief-toggle/);
...
assert.match(app, /MARKET_BRIEF_COLLAPSED_KEY/);
assert.match(app, /signalforge_market_brief_collapsed/);
assert.match(app, /function explainMarketCondition/);
assert.match(app, /The market is moving sideways between support and resistance/);
assert.match(app, /The possible reward is not large enough compared to the stop loss/);
assert.match(app, /daily-brief-collapsed-row/);
assert.match(app, /daily-brief-detail-card/);
assert.match(app, /renderScanResults\([\s\S]*?result\.marketBrief \|\| null/);
...
assert.match(css, /\.daily-brief-collapsed-row/);
assert.match(css, /\.daily-brief-detail-card/);
assert.match(css, /\.daily-brief-meaning/);
