...
const oldSignals = audit.signals.filter((signal) => signal.strategy === "VWAP reclaim/rejection");
...
for (const signal of oldSignals) {
  ...
  const remainsVwapReclaimRejection = directionCandidate?.setupType === "VWAP reclaim/rejection";
  ...
  records.push({
    ...
    newVwapReclaimRejection: remainsVwapReclaimRejection,
    strategyEvidence: remainsVwapReclaimRejection ? directionCandidate.strategyEvidence || null : null,
    fallthroughStrategy: remainsVwapReclaimRejection ? null : directionCandidate?.setupType || null,
    fallthroughValid: remainsVwapReclaimRejection
      ? false
...
const report = {
  methodology: {
    ...
    oldPopulation: "signals previously emitted as VWAP reclaim/rejection by scanMarketSetupDetailed",
    newQualification: "current ordered production classifier using canonical completed candles and same-session VWAP context",
    fallthrough: "generateMarketDataSetup continues through the unchanged ordered classifier and downstream validation",
    outcomeUse: "stored production-parity outcomes are retained only for signals that remain VWAP reclaim/rejection",
    tuning: false,
    noLookahead: true
  },
  ...
  removedOldVwapSignals: records.filter((record) => !record.newVwapReclaimRejection).length,
  remainingVwapSignals: records.filter((record) => record.newVwapReclaimRejection).length,
  fallthroughIntoAnotherStrategy: records.filter((record) => !record.newVwapReclaimRejection && record.fallthroughStrategy).length,
  fullyValidFallthrough: records.filter((record) => record.fallthroughValid).length,
  disappearEntirelyAtClassification: records.filter((record) => !record.newVwapReclaimRejection && !record.fallthroughStrategy).length,
  fallthroughStrategies: countBy(records.filter((record) => !record.newVwapReclaimRejection && record.fallthroughStrategy), "fallthroughStrategy"),
  records
};
...
function compare(recordsForPeriod) {
  const surviving = recordsForPeriod.filter((record) => record.newVwapReclaimRejection);
  return {
...
function buildMarketData(signal, manifest, candles) {
  ...
  return {
    ...
    source: "historical-vwap-reclaim-rejection-replay",
    candles,
