...
const oldSignals = audit.signals.filter((signal) => signal.strategy === "Liquidity sweep reversal");
...
for (const signal of oldSignals) {
  ...
  const evidence = evaluateLiquiditySweepReversalSetup(
    signal.direction,
    candles,
    { atr14: signal.atr },
    smcState
  ...
  records.push({
    ...
    newLiquiditySweepReversal: evidence.qualified,
    ambiguous: evidence.ambiguous,
    liquidityLevel: evidence.liquidityLevel,
    liquiditySide: evidence.liquiditySide,
    sweepDistanceAtr: evidence.sweepDistanceAtr,
    reclaimed: evidence.reclaimed,
    confirmationDirection: evidence.confirmationDirection,
    ageCandles: evidence.ageCandles,
    distanceFromSweepAtr: evidence.distanceFromSweepAtr,
    reversalMoveAtr: evidence.reversalMoveAtr,
...
const report = {
  methodology: {
    ...
    oldPopulation: "signals previously emitted as Liquidity sweep reversal by scanMarketSetupDetailed",
    newQualification: "evaluateLiquiditySweepReversalSetup at the original setup candle",
    ...
    outcomeUse: "stored production-parity outcomes are retained only for signals that remain Liquidity sweep reversal",
    tuning: false,
    noLookahead: true
  },
  ...
  removedOldLiquiditySweepReversals: records.filter((record) => !record.newLiquiditySweepReversal).length,
  fallthroughIntoAnotherStrategy: records.filter((record) => !record.newLiquiditySweepReversal && record.fallthroughStrategy).length,
  disappearEntirelyAtClassification: records.filter((record) => !record.newLiquiditySweepReversal && !record.fallthroughStrategy).length,
  fallthroughStrategies: countBy(
    records.filter((record) => !record.newLiquiditySweepReversal && record.fallthroughStrategy),
    "fallthroughStrategy"
  ),
  records
};
...
function compare(recordsForPeriod) {
  const surviving = recordsForPeriod.filter((record) => record.newLiquiditySweepReversal);
  return {
...
function buildMarketData(signal, manifest, candles, signalTime) {
  ...
  return {
    ...
    source: "historical-liquidity-sweep-replay",
    candles,
