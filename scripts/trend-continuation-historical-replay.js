...
const oldSignals = audit.signals.filter((signal) => signal.strategy === "Trend continuation");
...
for (const signal of oldSignals) {
  ...
  const remainsTrendContinuation = directionCandidate?.setupType === "Trend continuation";
  ...
  records.push({
    ...
    newTrendContinuation: remainsTrendContinuation,
    strategyEvidence: remainsTrendContinuation ? directionCandidate.strategyEvidence || null : null,
    fallthroughStrategy: remainsTrendContinuation ? null : directionCandidate?.setupType || null,
    fallthroughValid: remainsTrendContinuation
      ? false
...
const report = {
  methodology: {
    ...
    oldPopulation: "signals previously emitted as Trend continuation by scanMarketSetupDetailed",
    ...
    outcomeUse: "stored production-parity outcomes are retained only for signals that remain Trend continuation",
    tuning: false,
    noLookahead: true
  },
  ...
  removedOldTrendContinuations: records.filter((record) => !record.newTrendContinuation).length,
  remainingTrendContinuations: records.filter((record) => record.newTrendContinuation).length,
  fallthroughIntoAnotherStrategy: records.filter((record) => !record.newTrendContinuation && record.fallthroughStrategy).length,
  fullyValidFallthrough: records.filter((record) => record.fallthroughValid).length,
  disappearEntirelyAtClassification: records.filter((record) => !record.newTrendContinuation && !record.fallthroughStrategy).length,
  fallthroughStrategies: countBy(records.filter((record) => !record.newTrendContinuation && record.fallthroughStrategy), "fallthroughStrategy"),
  records
};
...
function compare(recordsForPeriod) {
  const surviving = recordsForPeriod.filter((record) => record.newTrendContinuation);
  return {
...
function buildMarketData(signal, manifest, candles) {
  ...
  return {
    ...
    source: "historical-trend-continuation-replay",
    candles,
