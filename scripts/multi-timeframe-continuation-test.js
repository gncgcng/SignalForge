...
for (const baseTimeframe of ["1h", "15m", "5m"]) {
  const long = fixture(baseTimeframe, "long", alignedFrames(baseTimeframe, "long"));
  const short = fixture(baseTimeframe, "short", alignedFrames(baseTimeframe, "short"));
  assert.equal(evaluate(long).passed, true, `${baseTimeframe} LONG should pass`);
  assert.equal(evaluate(short).passed, true, `${baseTimeframe} SHORT should pass`);
  assert.equal(classify(long), "Multi-timeframe continuation");
  assert.equal(classify(short), "Multi-timeframe continuation");
  assert.equal(evaluate(long).alignedCount, evaluate(short).alignedCount);
  assert.equal(evaluate(long).opposingCount, evaluate(short).opposingCount);
}
...
assert.equal(oldMtfRule(mixed), true, "characterization: one matching preferredDirection triggered the old rule");
...
assert.equal(evaluate(oneHour).passed, true, "1h has exactly one real higher timeframe and requires it to agree");
...
assert.equal(classify(fallthrough), "Pullback bounce");
...
console.log(JSON.stringify({
  oldRule: {
    ...
    nearestTimeframeWeight: null,
    broadestTimeframeWeight: null,
...
function oldMtfRule(item) {
  ...
  const candidateDirection = item.direction;
  ...
  const onePreferredDirection = item.confluenceContext.higherTimeframes.some((higher) => (
    higher.available && higher.regime?.preferredDirection === candidateDirection
  ));
