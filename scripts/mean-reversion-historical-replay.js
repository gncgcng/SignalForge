...
const oldSignals = audit.signals.filter((signal) => signal.strategy === "Mean reversion");
...
for (const signal of oldSignals) {
  ...
  const remainsMeanReversion = directionCandidate?.setupType === "Mean reversion";
  ...
  records.push({
    ...
    newMeanReversion: remainsMeanReversion,
    strategyEvidence: remainsMeanReversion ? directionCandidate.strategyEvidence || null : null,
    fallthroughStrategy: remainsMeanReversion ? null : directionCandidate?.setupType || null,
    fallthroughValid: remainsMeanReversion
      ? false
...
const report = {
  methodology: {
    ...
    oldPopulation: "signals previously emitted as Mean reversion by scanMarketSetupDetailed",
    ...
    outcomeUse: "stored production-parity outcomes are retained only for signals that remain Mean reversion",
    tuning: false,
    noLookahead: true
  },
  ...
  removedOldMeanReversions: records.filter((record) => !record.newMeanReversion).length,
  remainingMeanReversions: records.filter((record) => record.newMeanReversion).length,
  fallthroughIntoAnotherStrategy: records.filter((record) => !record.newMeanReversion && record.fallthroughStrategy).length,
  fullyValidFallthrough: records.filter((record) => record.fallthroughValid).length,
  disappearEntirelyAtClassification: records.filter((record) => !record.newMeanReversion && !record.fallthroughStrategy).length,
  fallthroughStrategies: countBy(records.filter((record) => !record.newMeanReversion && record.fallthroughStrategy), "fallthroughStrategy"),
  records
};
...
function compare(recordsForPeriod) {
  const surviving = recordsForPeriod.filter((record) => record.newMeanReversion);
  return {
...
function buildMarketData(signal, manifest, candles, signalTime) {
  ...
  return {
    ...
    source: "historical-mean-reversion-replay",
    candles,
