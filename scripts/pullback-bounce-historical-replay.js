...
const oldSignals = audit.signals.filter((signal) => signal.strategy === "Pullback bounce");
...
for (const signal of oldSignals) {
  ...
  const remainsPullbackBounce = directionCandidate?.setupType === "Pullback bounce";
  ...
  records.push({
    ...
    newPullbackBounce: remainsPullbackBounce,
    strategyEvidence: remainsPullbackBounce ? directionCandidate.strategyEvidence || null : null,
    fallthroughStrategy: remainsPullbackBounce ? null : directionCandidate?.setupType || null,
    fallthroughValid: remainsPullbackBounce
      ? false
...
const report = {
  methodology: {
    ...
    oldPopulation: "signals previously emitted as Pullback bounce by scanMarketSetupDetailed",
    ...
    outcomeUse: "stored production-parity outcomes are retained only for signals that remain Pullback bounce",
    tuning: false,
    noLookahead: true
  },
  ...
  removedOldPullbackBounces: records.filter((record) => !record.newPullbackBounce).length,
  remainingPullbackBounces: records.filter((record) => record.newPullbackBounce).length,
  fallthroughIntoAnotherStrategy: records.filter((record) => !record.newPullbackBounce && record.fallthroughStrategy).length,
  fullyValidFallthrough: records.filter((record) => record.fallthroughValid).length,
  disappearEntirelyAtClassification: records.filter((record) => !record.newPullbackBounce && !record.fallthroughStrategy).length,
  fallthroughStrategies: countBy(records.filter((record) => !record.newPullbackBounce && record.fallthroughStrategy), "fallthroughStrategy"),
  records
};
...
function compare(recordsForPeriod) {
  const surviving = recordsForPeriod.filter((record) => record.newPullbackBounce);
  return {
...
function buildMarketData(signal, manifest, candles, signalTime) {
  ...
  return {
    ...
    source: "historical-pullback-bounce-replay",
    candles,
