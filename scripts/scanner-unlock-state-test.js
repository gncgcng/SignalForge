...
const failedUnlock = await runFailedUnlock();
...
assert.deepEqual(firstUnlock.navigation, [], "Scanner unlock navigated away from Scanner.");
assert.deepEqual(middleUnlock.navigation, [], "Middle-card Scanner unlock navigated away from Scanner.");
assert.equal(firstUnlock.historySignalsLoaded, 5);
assert.equal(firstUnlock.historicalCardsInDesk, 0);
assert.equal(failedUnlock.scanResults.length, 4);
assert.equal(failedUnlock.renderNoSetupCalls, 0, "Failed unlock replaced the entire Signal Desk.");
assert.equal(failedUnlock.currentCardsInDesk, 4);
assert.deepEqual(failedUnlock.navigation, []);
assert.equal(rejectedUnlock.scanResults.length, 4);
assert.equal(rejectedUnlock.currentCardsInDesk, 4);
assert.equal(rejectedUnlock.status, "Unlock request rejected");
assert.deepEqual(rejectedUnlock.navigation, []);
assert.deepEqual(telegramUnlock.navigation, ["signals"], "Telegram-origin navigation behavior changed.");

const scanStartSource = extractBetween(
  appSource,
  'scanAllButton.addEventListener("click", async () => {',
  'cancelScanButton?.addEventListener("click"'
);
assert.match(scanStartSource, /state\.scanResults = \[\]/, "A new Scan All must remain able to replace the prior session.");

console.log(JSON.stringify({
  scannerFirstCard: summarize(firstUnlock, 0),
  scannerMiddleCard: summarize(middleUnlock, 2),
  failedUnlock: {
    retainedCards: failedUnlock.scanResults.length,
    renderNoSetupCalls: failedUnlock.renderNoSetupCalls,
    route: failedUnlock.route
  },
  rejectedUnlock: {
    retainedCards: rejectedUnlock.scanResults.length,
    status: rejectedUnlock.status,
    route: rejectedUnlock.route
  },
  telegramOrigin: {
    navigation: telegramUnlock.navigation
  },
  newScanAllReplacesPriorSession: true
}, null, 2));
...
async function runSuccessfulUnlock(index, source = "scanner") {
  ...
  const promise = harness.context.completeSignalUnlock({
    signal: unlocked,
...
async function runFailedUnlock(unlockError = null) {
  ...
  return {
    ...
    currentCardsInDesk: countMatches(harness.signalsGrid.innerHTML, "data-scan-card"),
    navigation: [...harness.navigation],
    route: harness.route.current,
...
function createHarness({ historySignal = null, unlockResponse = null, unlockError = null } = {}) {
  ...
  const unlockReveal = { classList: classListHarness() };
  ...
  const context = {
    ...
    document: {
      ...
      querySelector(selector) {
        if (selector === "#signal-count") return { textContent: "" };
        return null;
      }
    },
    api: {
    ...
    unlockReveal,
    highlightSignalKey: () => {},
    renderScanCard: (setup) => `<article data-scan-card="${setup.setupKey}"></article>`,
    renderSignalCard: (signal) => String(signal.id).startsWith("history-")
      ? `<article data-history-card="${signal.id}"></article>`
      : `<article data-unlocked-card="${signal.setupKey}"></article>`
  };
  ...
  vm.runInContext([
    ...
    extractBetween(
      ...
      "function mergeUnlockedSignalIntoScanResults"
    ),
...
function snapshot(harness) {
  return {
    scanResults: harness.state.scanResults.map((item) => ({ ...item })),
    currentCardsInDesk: countMatches(harness.signalsGrid.innerHTML, "data-scan-card") +
      countMatches(harness.signalsGrid.innerHTML, "data-unlocked-card"),
    historicalCardsInDesk: countMatches(harness.signalsGrid.innerHTML, "data-history-card")
  };
}
...
function summarize(result, index) {
  return {
    ...
    unlockedSetupKey: result.afterHistoryLoad.scanResults[index].setupKey,
