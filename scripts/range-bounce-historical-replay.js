...
const oldSignals = audit.signals.filter((signal) => signal.strategy === "Range bounce");
...
for (const signal of oldSignals) {
  ...
  const remainsRangeBounce = directionCandidate?.setupType === "Range bounce";
  ...
  records.push({
    ...
    newRangeBounce: remainsRangeBounce,
    strategyEvidence: remainsRangeBounce ? directionCandidate.strategyEvidence || null : null,
    fallthroughStrategy: remainsRangeBounce ? null : directionCandidate?.setupType || null,
    fallthroughValid: remainsRangeBounce
      ? false
...
const report = {
  methodology: {
    ...
    oldPopulation: "signals previously emitted as Range bounce by scanMarketSetupDetailed",
    ...
    outcomeUse: "stored production-parity outcomes are retained only for signals that remain Range bounce",
    tuning: false,
    noLookahead: true
  },
  ...
  removedOldRangeBounces: records.filter((record) => !record.newRangeBounce).length,
  remainingRangeBounces: records.filter((record) => record.newRangeBounce).length,
  fallthroughIntoAnotherStrategy: records.filter((record) => !record.newRangeBounce && record.fallthroughStrategy).length,
  fullyValidFallthrough: records.filter((record) => record.fallthroughValid).length,
  disappearEntirelyAtClassification: records.filter((record) => !record.newRangeBounce && !record.fallthroughStrategy).length,
  fallthroughStrategies: countBy(records.filter((record) => !record.newRangeBounce && record.fallthroughStrategy), "fallthroughStrategy"),
  records
};
...
function compare(recordsForPeriod) {
  const surviving = recordsForPeriod.filter((record) => record.newRangeBounce);
  return {
...
function buildMarketData(signal, manifest, candles, signalTime) {
  ...
  return {
    ...
    source: "historical-range-bounce-replay",
    candles,
