...
assert.match(appSource, /\[auth-ui\] login:response_status=/);
...
assert.match(appSource, /if \(isAuthOperationCurrent\(operationId\)\) saveAuthSession\(session\)/);
assert.match(appSource, /if \(!isAuthOperationCurrent\(operationId\)\) return null;\s+saveAuthSession\(result\)/);
...
assert.match(authService, /login:password_check:valid=/);
